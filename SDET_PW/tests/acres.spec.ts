...
//TO VALIDATE BORROW AMNT CALCULATION
...
//TO CHECK CORRECT EMI CALCULATION
...
//TO VALIDATE EMAIL IN CALLBACK FORM
...
//TO VALIDATE IF EMAIL FORM IS VISIBLE
...
test("Check keyboard typability in email fields", async ({page}) => {
  ...
  //Choose Field of HDFC (it is the only one active in the site)
  await page.locator("//span[text()='HDFC']/parent::div/following-sibling::div/child::div[@class='dealButton']/child::span[text()='Email me']").click();
...
//TO VALIDATE THE URL REDIRECT
test.only('Check page redirect', async({ browser}) =>{
    ...
    await expect(newPage).toHaveURL("https://www.hdfc.bank.in/important-messages/product-features-and-policy");
});